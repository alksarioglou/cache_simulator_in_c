...
#define ACCESS_TIME  110   // Main Memory Access Time
#define HIT_TIME  10          // Cache Memory Access Time
...
float t=0; // Execution Time in ns

// Main Function
int main (void) {

    // Clear Results File
    fp_write = fopen ("Results_Alkinoos_Sarioglou_10136315.csv", "w");
...
void cache_analysis(char str[100],unsigned int mode_id) {
    ...
    fp_write = fopen ("Results_Alkinoos_Sarioglou_10136315.csv", "a");
    if (fp_write == NULL) printf ("Error with the results file");
        ...
        // Read Operation
            ...
            for (int i=0;i<no_cache_lines;i++) {
                    ...
                    if (valid_bit[i] == false) {
                        count++;                   // If contents of cache line not valid, do not compare
                        continue;
                    }
            ...
            if (count==no_cache_lines){      // Cache Read Miss
                ...
                if (dirty_bit[round_robin_index]==1) {      // Write block to replace to memory if something was written to it
                    ...
                    t+=(ACCESS_TIME*block_size);          // Write Access Time
                    ...
                    t+=(ACCESS_TIME*block_size);  // Write and Read Access Time
                }
                // If dirty bit is LOW
                else {
                    ...
                    nra+=block_size;            // Increment Number of Read Accesses to Main Memory by the block size
                    t+=(ACCESS_TIME*block_size);  // Read Access Time
                }
    ...
    printf("\n %s, %d, %d, %d, %d, %d, %d, %d, time: %.2f ms", str, mode_id, nra, nwa, ncrh, ncrm, ncwh, ncwm, (0.000001)*(t));    // Print results to the terminal window

    fprintf(fp_write, "%s, %d, %d, %d, %d, %d, %d, %d\n", str, mode_id, nra, nwa, ncrh, ncrm, ncwh, ncwm);  // Write the results to the .csv file
